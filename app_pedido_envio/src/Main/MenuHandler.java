...
public class MenuHandler {
     ...
     */
    private final EnvioServiceImpl personaService;
     ...
     */
    public MenuHandler(Scanner scanner, EnvioServiceImpl personaService) {
        if (scanner == null) {
    ...
    public void crearPersona() {
        try {
            ...
            Envio persona = new Envio(0, nombre, apellido, dni);
    ...
    public void actualizarPersona() {
        try {
            ...
            Envio p = personaService.getById(id);
    ...
    public void crearDomicilioIndependiente() {
        try {
            Pedido domicilio = crearDomicilio();
    ...
    public void listarDomicilios() {
        try {
            List<Pedido> domicilios = personaService.getDomicilioService().getAll();
            if (domicilios.isEmpty()) {
                System.out.println("No se encontraron domicilios.");
                return;
            }
            for (Pedido d : domicilios) {
    ...
    public void actualizarDomicilioPorId() {
        try {
            ...
            int id = Integer.parseInt(scanner.nextLine());
            Pedido d = personaService.getDomicilioService().getById(id);
    ...
    public void actualizarDomicilioPorPersona() {
        try {
            ...
            Envio p = personaService.getById(personaId);
     ...
     *
     * @return Pedido nuevo (no persistido, ID=0)
     */
    private Pedido crearDomicilio() {
        System.out.print("Calle: ");
        String calle = scanner.nextLine().trim();
        System.out.print("Numero: ");
        String numero = scanner.nextLine().trim();
        return new Pedido(0, calle, numero);
    }
     ...
     *
     * @param p Envio a la que se le actualizará/agregará domicilio
     * @throws Exception Si hay error al insertar/actualizar domicilio
     */
    private void actualizarDomicilioDePersona(Envio p) throws Exception {
        if (p.getDomicilio() != null) {
            ...
            if (scanner.nextLine().equalsIgnoreCase("s")) {
                Pedido nuevoDom = crearDomicilio();
